...
@asyncio.coroutine
def sensehat_read_temperature(keys):
    try:
        sample = sense.get_temperature()
        return [ sample for k in keys ]
    except:
        return []
...
def main(loop, host, plugin, interval):
    """Main loop."""
    while loop.jk_run:
        res = yield from sensehat_read_temperature([ m['key'] for m in METRICS ])
        t = time.time()
